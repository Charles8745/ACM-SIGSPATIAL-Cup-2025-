...
if __name__ == "__main__": 
    ...
    #             best_loss = avg_loss
    #             wait = 0
    #             os.makedirs(f"./ckpt/CVAE_linearweight/uid_level_class/", exist_ok=True)
    #             torch.save(model.state_dict(), f"./ckpt/CVAE_linearweight/uid_level_class/cvae_model_uid{valid_uid}_l{latent_dim}h{hidden_dim}_city{city}.pth")
    #         else:
    #             wait += 1
    #             if wait >= patience:
    ...
    for idx, uid in enumerate(valid_uid_list):
        ...
        ckpt_path = f"./ckpt/CVAE_linearweight/uid_level_class/cvae_model_uid{uid}_l{latent_dim}h{hidden_dim}_city{city}.pth"
    ...
    os.makedirs('./Predictions/CVAE_linearweight/', exist_ok=True)
    pred_df.to_csv(f'./Predictions/CVAE_linearweight/{city}_x_cvae_pred.csv', index=False)
    print(f"已輸出預測結果至 ./Predictions/CVAE_linearweight/{city}_x_cvae_pred.csv")

    # ===== scatter cvae 61-75天 vs. gt 61-75 =====
    mode_pred_df = pd.read_csv(f'./Predictions/{city}_x_Per_User_Per_t_Mode_working_day.csv')
    cvae_pred_df = pd.read_csv(f'./Predictions/CVAE_linearweight/{city}_x_cvae_pred.csv')
