...
"""
針對個人去訓練的 CVAE 模型
Ablation Study: 特徵輸入改成 [uid+t+working_day+day_of_week]，新增 day_of_week
"""
class TrajectoryDataset(Dataset):
    def __init__(self, df, uid_list, xy2idx, max_len=400):
        ...
        for uid in uid_list:
            ...
            # ===== MODIFIED: 新增 day_of_week =====
            traj = user_df[['x', 'y', 't', 'working_day', 'day_of_week']].values
            # ===== END MODIFIED =====
            length = len(traj)
    ...
    def __getitem__(self, idx):
        ...
        # ===== MODIFIED: 新增 day_of_week_seq =====
        day_of_week_seq = traj[:, 4].long()
        # ===== END MODIFIED =====
        weights = self.weights[idx]
        xy_idx = self.xy_idx_list[idx]
        # ===== MODIFIED: 回傳 day_of_week_seq =====
        return traj, mask, length, uid, t_seq, working_day_seq, day_of_week_seq, weights, xy_idx
        # ===== END MODIFIED =====
...
class CVAE(nn.Module):
    def __init__(self, input_dim, latent_dim, uid_dim, uid_embed_dim, hidden_dim, max_len, N_valid, num_layers=1, dropout=0.3):
        ...
        # ===== MODIFIED: 新增 day_of_week_embedding (0-6, 7天) =====
        self.day_of_week_embedding = nn.Embedding(7, 7)
        # ===== END MODIFIED =====
        # ===== MODIFIED: encoder_rnn 輸入維度改為 input_dim + uid_embed_dim + 24 + 2 + 7 =====
        self.encoder_rnn = nn.LSTM(input_dim + uid_embed_dim + 24 + 2 + 7, hidden_dim, batch_first=True, num_layers=num_layers, dropout=dropout if num_layers > 1 else 0)
        # ===== END MODIFIED =====
        self.encoder_fc = nn.Linear(hidden_dim, latent_dim * 2)
        # ===== MODIFIED: decoder_rnn 輸入維度改為 latent_dim + uid_embed_dim + 24 + 2 + 7 =====
        self.decoder_rnn = nn.LSTM(latent_dim + uid_embed_dim + 24 + 2 + 7, hidden_dim, batch_first=True, num_layers=num_layers, dropout=dropout if num_layers > 1 else 0)
        # ===== END MODIFIED =====
    ...
    def encode(self, x, uid, t, working_day, day_of_week, mask):
        ...
        # ===== MODIFIED: 新增 day_of_week embedding =====
        dow_embed = self.day_of_week_embedding(day_of_week)
        # ===== END MODIFIED =====
        uid_embed_expand = uid_embed.unsqueeze(1).expand(-1, x.size(1), -1)
        x_wo_t = x[..., :2]
        # ===== MODIFIED: 連接 x, uid, t, working_day, day_of_week =====
        x_cat = torch.cat([x_wo_t, uid_embed_expand, t_embed, wd_embed, dow_embed], dim=-1)
        # ===== END MODIFIED =====
    ...
    def decode(self, z, uid, t, working_day, day_of_week):
        ...
        # ===== MODIFIED: 連接 z, uid, t, working_day, day_of_week =====
        z_cat_seq = torch.cat([z_expand, uid_embed_expand, t_embed, wd_embed, dow_embed], dim=-1)
        # ===== END MODIFIED =====
    ...
    def forward(self, x, uid, t, working_day, day_of_week, mask):
        ...
        xy_logits = self.decode(z, uid, t, working_day, day_of_week)
...
def generate_future_trajectory(model, user_train_df, user_test_df, uid_idx, device, valid_xy_list, temperature=1.0, top_k=None, top_p=None):
    ...
    with torch.no_grad():
        ...
        # ===== MODIFIED: 新增 day_of_week_seq_train =====
        dow_seq_train = torch.tensor(traj[:, 4], dtype=torch.long).unsqueeze(0).to(device)
        # ===== END MODIFIED =====

        mu, logvar = model.encode(x, uid_tensor, t_seq_train, wd_seq_train, dow_seq_train, mask)
        ...
        # ===== MODIFIED: 新增 day_of_week_seq_gen =====
        dow_seq_gen = torch.tensor(user_test_df['day_of_week'].values, dtype=torch.long).unsqueeze(0).to(device)
        # ===== END MODIFIED =====
        xy_logits = model.decode(z, uid_tensor, t_seq_gen, wd_seq_gen, dow_seq_gen)
...
if __name__ == "__main__": 
    ...
    #         total_recon = 0
    #         total_kl = 0
    #         # ===== MODIFIED: dataloader 解包增加 day_of_week =====
    #         for x, mask, lengths, uid, t, working_day, day_of_week, weights, xy_idx in dataloader:
    #         # ===== END MODIFIED =====
    #             x = x.to(device)
    #             mask = mask.to(device)
    #             t = t.to(device)
    #             working_day = working_day.long().to(device)
    #             # ===== MODIFIED: day_of_week 移到 device =====
    #             day_of_week = day_of_week.long().to(device)
    #             # ===== END MODIFIED =====
    #             # 單 uid 設為索引 0，避免嵌入越界
    ...
    #             optimizer.zero_grad()
    #             # ===== MODIFIED: model forward 傳入 day_of_week =====
    #             xy_logits, mu, logvar = model(x, uid, t, working_day, day_of_week, mask)
    #             # ===== END MODIFIED =====
    ...
    #             best_loss = avg_loss
    #             wait = 0
    #             os.makedirs(f"./ckpt/CVAE_UidTDayofweek/uid_level_class/", exist_ok=True)
    #             torch.save(model.state_dict(), f"./ckpt/CVAE_UidTDayofweek/uid_level_class/cvae_model_uid{valid_uid}_l{latent_dim}h{hidden_dim}_city{city}.pth")
    #         else:
    #             wait += 1
    #             if wait >= patience:
    ...
    for idx, uid in enumerate(valid_uid_list):
        ...
        ckpt_path = f"./ckpt/CVAE_UidTDayofweek/uid_level_class/cvae_model_uid{uid}_l{latent_dim}h{hidden_dim}_city{city}.pth"
    ...
    os.makedirs('./Predictions/CVAE_UidTDayofweek/', exist_ok=True)
    pred_df.to_csv(f'./Predictions/CVAE_UidTDayofweek/{city}_x_cvae_pred.csv', index=False)
    print(f"已輸出預測結果至 ./Predictions/CVAE_UidTDayofweek/{city}_x_cvae_pred.csv")

    # ===== scatter cvae 61-75天 vs. gt 61-75 =====
    mode_pred_df = pd.read_csv(f'./Predictions/{city}_x_Per_User_Per_t_Mode_working_day.csv')
    cvae_pred_df = pd.read_csv(f'./Predictions/CVAE_UidTDayofweek/{city}_x_cvae_pred.csv')
