...
"""
針對個人去訓練的 CVAE 模型
Ablation Study: 特徵輸入改成 [uid+t]，移除 working_day
"""
...
class CVAE(nn.Module):
    def __init__(self, input_dim, latent_dim, uid_dim, uid_embed_dim, hidden_dim, max_len, N_valid, num_layers=1, dropout=0.3):
        ...
        # ===== MODIFIED: 移除 working_day_embedding =====
        # self.working_day_embedding = nn.Embedding(2, 2)  # 移除
        # ===== END MODIFIED =====
        # ===== MODIFIED: encoder_rnn 輸入維度改為 input_dim + uid_embed_dim + 24 =====
        self.encoder_rnn = nn.LSTM(input_dim + uid_embed_dim + 24, hidden_dim, batch_first=True, num_layers=num_layers, dropout=dropout if num_layers > 1 else 0)
        # ===== END MODIFIED =====
        self.encoder_fc = nn.Linear(hidden_dim, latent_dim * 2)
        # ===== MODIFIED: decoder_rnn 輸入維度改為 latent_dim + uid_embed_dim + 24 =====
        self.decoder_rnn = nn.LSTM(latent_dim + uid_embed_dim + 24, hidden_dim, batch_first=True, num_layers=num_layers, dropout=dropout if num_layers > 1 else 0)
        # ===== END MODIFIED =====
    ...
    def encode(self, x, uid, t, working_day, mask):
        ...
        # ===== MODIFIED: 移除 working_day embedding =====
        # wd_embed = self.working_day_embedding(working_day)  # 移除
        # ===== END MODIFIED =====
        uid_embed_expand = uid_embed.unsqueeze(1).expand(-1, x.size(1), -1)
        x_wo_t = x[..., :2]
        # ===== MODIFIED: 只連接 x, uid, t =====
...
if __name__ == "__main__": 
    ...
    #             best_loss = avg_loss
    #             wait = 0
    #             os.makedirs(f"./ckpt/CVAE_UidT/uid_level_class/", exist_ok=True)
    #             torch.save(model.state_dict(), f"./ckpt/CVAE_UidT/uid_level_class/cvae_model_uid{valid_uid}_l{latent_dim}h{hidden_dim}_city{city}.pth")
    #         else:
    #             wait += 1
    #             if wait >= patience:
    ...
    for idx, uid in enumerate(valid_uid_list):
        ...
        ckpt_path = f"./ckpt/CVAE_UidT/uid_level_class/cvae_model_uid{uid}_l{latent_dim}h{hidden_dim}_city{city}.pth"
    ...
    os.makedirs('./Predictions/CVAE_UidT/', exist_ok=True)
    pred_df.to_csv(f'./Predictions/CVAE_UidT/{city}_x_cvae_pred.csv', index=False)
    print(f"已輸出預測結果至 ./Predictions/CVAE_UidT/{city}_x_cvae_pred.csv")

    # ===== scatter cvae 61-75天 vs. gt 61-75 =====
    mode_pred_df = pd.read_csv(f'./Predictions/{city}_x_Per_User_Per_t_Mode_working_day.csv')
    cvae_pred_df = pd.read_csv(f'./Predictions/CVAE_UidT/{city}_x_cvae_pred.csv')
