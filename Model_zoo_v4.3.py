...
def generate_future_trajectory(model, user_train_df, user_test_df, uid, device,temperature=1.0, top_k=None, top_p=None):
    ...
    with torch.no_grad():
        ...
        # 使用機率抽樣
        x_pred = sample_from_logits(x_logits[:, :gen_len, :], temperature, top_k, top_p).squeeze(0).cpu().numpy() + 1
        y_pred = sample_from_logits(y_logits[:, :gen_len, :], temperature, top_k, top_p).squeeze(0).cpu().numpy() + 1
