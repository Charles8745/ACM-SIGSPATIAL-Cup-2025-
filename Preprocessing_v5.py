...
class DataPreprocessor:
    ...
    def get_training_testing_data(self, city_name, data_input_path):
        ...
        v5增加delta_t欄位，表示每個uid在每個時間點與上個時間點的時間差。
        """
        ...
        cross_day = (raw_data_df['uid'] == raw_data_df['uid'].shift()) & (raw_data_df['d'] != raw_data_df.groupby('uid')['d'].shift())
        # 用向量化方式直接處理所有跨天的delta_t
        prev_t = raw_data_df['t'].shift()
        raw_data_df.loc[cross_day, 'delta_t'] = raw_data_df.loc[cross_day, 't'] + 48 - prev_t[cross_day]
        # 第一筆資料（沒有前一筆）設為1
    ...
    def extract_features(self, input_x_train, input_y_train, output_name='Cityname', valid_uid_list=None):  
        ...
        for uid, group in df.groupby('uid'):
            ...
            # 上班路徑
            morning_df = group[(group['working_day']==1) & (group['t']<=24)]
            commute_paths = []
            for day, group in morning_df.groupby('d'):
                group = group.sort_values('t')
                coords = group[['x', 'y']].values

                # 找出離home最近的index（出發點）
                dists_home = np.linalg.norm(coords - np.array(home_loc), axis=1)
                home_idx = np.where(dists_home <= 4)[0]
                if len(home_idx) == 0:
                    continue
                start_idx = home_idx[-1]  # 最後一次還在家的index

                # 找出離work最近的index（到達點）
                dists_work = np.linalg.norm(coords - np.array(work_loc), axis=1)
                work_idx = np.where((dists_work <= 4) & (np.arange(len(coords)) > start_idx))[0]
                if len(work_idx) == 0:
                    continue
                end_idx = work_idx[0]  # 第一次到公司的index

                # 取出通勤路徑
                commute_path = group.iloc[start_idx:end_idx+1]
                commute_paths.append(commute_path)
            # 統計所有通勤路徑點的出現次數，只保留出現2次以上的點
            all_points = []
            for path in commute_paths:
                all_points.extend(list(zip(path['x'], path['y'])))
            point_counts = Counter(all_points)
            frequent_points = {pt for pt, cnt in point_counts.items() if cnt >= 2} # 只保留出現2次以上的點
            # 過濾通勤路徑，只保留包含頻繁點的
            filtered_commute_paths = []
            for path in commute_paths:
                filtered = path[path.apply(lambda row: (row['x'], row['y']) in frequent_points, axis=1)]
                if not filtered.empty:
                    filtered_commute_paths.append(filtered)
            commute_paths = filtered_commute_paths
            # 將所有通勤路徑點合併，去除重複，一個點只保留一份
            seen = set()
            unique_commute_points = []
            for path in commute_paths:
                for _, row in path.iterrows():
                    pt = (row['x'], row['y'])
                    if pt not in seen:
                        seen.add(pt)
                        unique_commute_points.append(pt)
            # 依據離home的距離排序，越近排前面
            unique_commute_points = sorted(
                unique_commute_points,
                key=lambda pt: np.linalg.norm(np.array(pt) - np.array(home_loc)))
            # 儲存特徵
            features.append({
                'uid': uid,
                ...
                'commute_paths': ';'.join([f"({int(x)},{int(y)})" for x, y in unique_commute_points]),
