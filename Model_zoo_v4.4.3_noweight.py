...
"""
針對個人去訓練的 CVAE 模型
Ablation Study 2: No Weight (改成無權重)
"""
class TrajectoryDataset(Dataset):
    def __init__(self, df, uid_list, xy2idx, max_len=400):
        ...
        for uid in uid_list:
            ...
            xy = traj[:, :2]
            # ===== MODIFIED: 無權重，所有權重設為1.0 =====
            weights_full = np.ones(max_len, dtype=np.float32)
            # ===== END MODIFIED =====
...
def cvae_loss(xy_logits, xy_idx, mu, logvar, mask, weights, beta=0.05, lambda_entropy=0.01):
    ...
    mask = mask.view(-1)
    # ===== MODIFIED: 無權重版本，不使用weights =====
    # weights = weights.view(-1)  # 不需要

    valid = mask > 0
    loss_fn = nn.CrossEntropyLoss(reduction='mean')  # 改為 mean
    ce_loss = loss_fn(xy_logits[valid], xy_idx[valid])
    weighted_loss = ce_loss  # 直接使用 ce_loss
    # ===== END MODIFIED =====
